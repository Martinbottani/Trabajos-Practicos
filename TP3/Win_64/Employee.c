...
/**
    employee_cargarEmpleado: Agrega nuevos datos al array LinkedList.
...
int employee_modificarEmployee(LinkedList* pArrayEmployee)
{
    ...
    int auxSueldo;
    Employee* auxPunteroEmployee;
    if(pArrayEmployee != NULL && utn_getEnteroSinLimites(&idEmployee, 10, "\nIngrese el ID del empleado a modificar: \n", "\nError, id invalido\n") == 0)
    {
        indice = employee_buscarPorId(pArrayEmployee, idEmployee);
        if(indice >= 0)
        {
            auxPunteroEmployee = ll_get(pArrayEmployee, indice);
            if( utn_getNombre(auxNombre, "\nIngrese el nuevo nombre del empleado: \n","\nError, nombre invalido\n")==0 &&
                utn_getEnteroSinLimites(&auxHorasTrabajadas, 10, "\nIngrese la nueva cantidad de horas trabajadas: \n", "\nError, horas de trabajo invalidas\n") == 0 &&
                utn_getEnteroSinLimites(&auxSueldo, 10,"\nIngrese el nuevo sueldo: \n", "\nError, sueldo invalido\n") == 0)
            {
                auxPunteroEmployee->horasTrabajadas = auxHorasTrabajadas;
                auxPunteroEmployee->sueldo = auxSueldo;
                strcpy(auxPunteroEmployee->nombre, auxNombre);
                retorno = 0;
            }
        }else
        {
...
int employee_borrarEmployee(LinkedList* pArrayEmployee)
{
    ...
    if(pArrayEmployee != NULL && utn_getEnteroSinLimites(&idEmployee, 10, "\nIngrese el ID del empleado a borrar: \n", "\nError, id invalido\n") == 0)
    {
        ...
        if(id >= 0)
        {
            auxPunteroEmployee = ll_get(pArrayEmployee,id);
            if(employee_delete(auxPunteroEmployee) == 0)
            {
                retorno = 0;
...
int employee_guardarDatosEnTexto(LinkedList* pArrayEmployee, char* path)
{
    ...
    if(pFile != NULL)
    {
        ...
        for(i = 0; i < size; i++)
        {
            auxPunteroEmployee = ll_get(pArrayEmployee,i);
            fprintf(pFile, "%d,%s,%d,%d\n", auxPunteroEmployee->id, auxPunteroEmployee->nombre, auxPunteroEmployee->horasTrabajadas, auxPunteroEmployee->sueldo);
        }
    }
    fclose(pFile);
    return retorno;
...
int employee_guardarDatosEnBinario(LinkedList* pArrayEmployee, char* path)
{
    ...
    if(pFile != NULL)
    {
        ...
        for(i = 0; i < size; i++)
        {
            auxPunteroEmpleado = ll_get(pArrayEmployee, i);
            fwrite(auxPunteroEmpleado, sizeof(Employee), 1, pFile);
        }
    }
    fclose(pFile);
    return retorno;
